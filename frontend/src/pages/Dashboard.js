...
const Dashboard = () => {
  ...
  const handleSessionChange = (e) => {
    ...
    setNewSession((prev) => ({
      ...prev,
      [name]:
        name === "productivity_level" || name === "hours_studied"
          ? Number(value)
          : value,
    }));
  };
  ...
  const calculateStats = () => {
    const totalHours = weekSessions.reduce(
      (sum, session) => sum + session.hours_studied,
      0,
    );
    const avgProductivity =
      weekSessions.length > 0
        ? (
            weekSessions.reduce(
              (sum, session) => sum + session.productivity_level,
              0,
            ) / weekSessions.length
          ).toFixed(1)
    ...
    const productiveSessions = todaySessions.filter(
      (session) => session.hours_studied * 60 > 40,
    );
    const unproductiveSessions = todaySessions.filter(
      (session) => session.hours_studied * 60 < 20,
    );
                      ...
                      <div key={idx} className="session-item">
                        <div className="session-info">
                          ...
                          <p className="session-time">
                            ...
                            {session.hours_studied.toFixed(1)}h (
              ...
              <div className="heatmap-container">
                {["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map(
                  (day, idx) => {
                    ...
                    dayDate.setDate(
                      dayDate.getDate() - dayDate.getDay() + idx + 1,
                    );
                    ...
                    const color =
                      intensity === 0
                        ? "#e0e0e0"
                        : `hsl(265, ${70 + intensity * 20}%, ${50 - intensity * 15}%)`;
            ...
            {(weeklyReport && (
              <div className="card">
                ...
                <div className="readiness-report">
                  <div className="readiness-score">
                    ...
                    <span className="score-value">
                      {weeklyReport.exam_readiness_score || 0}%
                    </span>
                  </div>
                ...
                <div className="summary-stats">
                  Sessions: {weeklyReport.total_sessions || 0} • Avg
                  Productivity:{" "}
                  {weeklyReport.avg_productivity?.toFixed(1) || "0.0"}/5
                </div>
            ...
            <div className="card">
              ...
              {nextDayTimetable && (
                <div className="timetable">
                  {nextDayTimetable.timetable &&
                    nextDayTimetable.timetable.map((slot, idx) => (
          ...
          <div className="dashboard-page">
              ...
              {subjects.length > 0 ? (
                <div className="performance-grid">
                  {subjects.map((subject) => {
                    ...
                    const hours = subjectSessions.reduce(
                      (sum, s) => sum + s.hours_studied,
                      0,
                    );
                    const avgProductivity = (
                      subjectSessions.reduce(
                        (sum, s) => sum + s.productivity_level,
                        0,
                ...
                <p className="no-data">
                  No data available. Start logging sessions!
                </p>
              )}
            </div>
            ...
            <div className="card">
              ...
              <div className="trend-info">
                ...
                <p>
                  ⏱️ <strong>Total Hours:</strong> {stats.totalHours}h
                </p>
                <p>
                  ⭐ <strong>Avg Productivity:</strong> {stats.avgProductivity}
                  /5
                </p>
              </div>
            </div>
